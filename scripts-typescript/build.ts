...
const nginx = (port: number): string => `# nginx/nginx.conf
events {
...
const dockerfile = (port: number): string => `
FROM debian:latest
...
const dockerCompose = (port: number): string => `
services:
  debian_nginx:
    build: .
    ports:
...
const makefile = (port: number): string => `
all: up
...
(async (): Promise<void> => {
  try {
    ...
    let port = config.port || 17000;
    loginfo("available port", port);
  
  ["./docker/app", "./docker/nginx"].forEach((subDir) => {
    try {
