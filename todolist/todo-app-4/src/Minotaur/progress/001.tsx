...
import { validTags } from "../utils.js";
...
function reconciliate(left: VDOM, right: VDOM) {
  ...
  for (let i = 0; i < Math.max(children1.length, children2.length); i++) {
    const child1: VDOMNode = children1[i];
    const child2: VDOMNode = children2[i];

    if (child1 && child2) {
      let diff = reconciliate(child1 as VDOM, child2 as VDOM);
...
function setProps(vdom, parent) {
  ...
  Object.keys(props || {}).forEach((key) => {
    ...
    if (key.startsWith("on")) {
      ...
      if (tag == "svg" || parent?.tag == "svg") vdom.dom.setAttribute(key, props[key]);
...
function render(rec, parent: VDOM) {
  ...
  switch (rec.action) {
    ...
    case "insert": {
      ...
      rec.right.children.map((child) => {
        ...
        display(child as VDOM, rec.right);
        //@ts-ignore
