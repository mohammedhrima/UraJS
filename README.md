...
7. **all commands**:
```bash
  urastart #start server
```
```bash
  uraclear #clear outfile
```
```bash
  uraroute #create route
```
```bash
  uracomp #create component
```
```bash
  urabuild #to build
```
...
- To generate a **nested route and its SCSS file**, run:
    ...
    ```bash
      uraroute /routename/nestedroute
    ```
...
+ When you run uraroute Component, the generated JSX code looks like this:
```js
    import Ura from 'ura';
